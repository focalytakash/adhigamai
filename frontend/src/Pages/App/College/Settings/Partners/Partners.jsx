...
function Partners() {
    ...
    const fetchPartners = async () => {
        try {
            setLoading(true);
            const response = await fetch(`${backendUrl}/college/users/partners`, {
                method: 'GET',
                headers: {
                    'x-auth': token,
            ...
            if (data.status || data.success) {
                ...
                showAlert(data.message || 'Failed to fetch partners', 'error');
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const url = isEditing
                ? `${backendUrl}/college/users/updatePartner/${editingId}`
                : `${backendUrl}/college/users/addPartner`;
            ...
            showAlert('Failed to save partner', 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleStatusToggle = async (partnerId, currentStatus) => {
        try {
            ...
            const response = await fetch(`${backendUrl}/college/users/updatePartner/${partnerId}`, {
                method: 'PUT',
                headers: {
                    'x-auth': token,
                    'Content-Type': 'application/json'
            ...
            if (data.status || data.success) {
                // Update local state
                setUsers(prev => prev.map(partner =>
